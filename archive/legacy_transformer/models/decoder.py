...
# 活动类型转换约束矩阵（软约束：-inf 表示禁止，正值表示鼓励，0 表示中性）
# 行=当前类型，列=下一类型
ACTIVITY_TRANSITION_CONSTRAINTS = torch.tensor([
    # 景点  餐饮  住宿  交通  购物  出发点
    [  0.0,  2.0, -1e9,  0.0,  1.0,  0.0],  # 景点后：鼓励餐饮/购物，禁止住宿
    [  2.0, -1e9,  0.0,  0.0,  0.0, -1e9],  # 餐饮后：鼓励景点，不能连续餐饮
    [  2.0,  0.0, -1e9,  0.0,  0.0,  0.0],  # 住宿后（起点）：优先去景点
    [  2.0,  0.0, -1e9, -1e9,  0.0,  0.0],  # 交通后：优先去景点，不能连续交通
    [  2.0,  0.0, -1e9,  0.0, -1e9,  0.0],  # 购物后：优先回景点
    [  2.0,  0.0, -1e9,  0.0,  0.0, -1e9],  # 出发点后：优先去景点
], dtype=torch.float32)
