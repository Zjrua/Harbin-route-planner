...
## 五轮评估实验：LLM 在候选选择中的价值裁决

一套完整的证据链（全部可由 `scripts/eval_*.py` + `output/*.json` 复现），核心问题是：**微调 LLM 在"从候选池选 POI"上相对简单规则的真实边际价值是多少？**

| # | 实验 | 设计 | 结果 |
|---|---|---|---|
| 1 | 无 LLM 基线（`eval_llm_vs_rule`） | 60 指令 × 3 seed 生成任务，配对 Wilcoxon | LLM 0.654 ≈ 贪心最近 0.645（p=0.95）；两者都显著 > 天真取前 N |
| 2 | 下一站预测（`eval_next_poi`） | 447 个真实游记预测点（8 选 1），McNemar | **总分规则赢**：rule_near 34.7% vs LLM 24.2%（p=0.0003） |
| 3 | 分桶分析（`eval_next_poi_buckets`） | 按真实下一站距离分桶 | **辛普森悖论反转**：近<2km（52% 份额）规则 66% vs LLM 29%；中远距（48% 份额）规则 **0%** vs LLM **19.7%**（p<1.8e-12） |
| 4 | α 线性混合（`eval_hybrid_selector`） | α·就近分 + (1-α)·LLM 概率，扫 α | 失败：纯权衡无双赢点，最优 α=0.9 ≈ 纯规则（p=0.44） |
| 5 | 门控选择器（`eval_gated_selector`） | 候选池中位距离门控（AUC 0.81），train/test 分半 | test 33.9% vs 规则 33.0%（p=0.82 不显著）；train 42.6% 系过拟合 |

### 核心发现

1. **真实游客行为是双峰的**：52% 的转移是商圈内就近闲逛（<2km），48% 是跨区转移（>2km，p90 达 7km+）。**推荐系统的价值恰在后者**——为地标跨城、换片区，而这正是纯就近规则完全无能（0% 命中）、LLM 显著有能的部分。
2. **但工程兑现比发现难**：双峰是**模式切换**而非连续偏好——线性混合被证明是错误的函数形式；单特征门控的判别力（AUC 0.81）不足以覆盖"错误路由近桶"的代价（近桶 52% 份额 × 66% 基线）。
3. **方法论教训**：三个评估陷阱被依次踩中又被纠正——①打分器与基线同源（v5 的 0.45 权重奖励就近，贪心最近天然最优）；②聚合谬误/辛普森悖论（总分被大组绑架）；③train/test 过拟合（门控阈值）。**分层分析与配对检验是这轮工作的方法论核心产出。**

### 架构含义

- 生成管线维持"检索 + LLM 排序 + 规则节奏层"（生成质量与规则打平、且提供多样性），但**模型侧不再追加训练投入**
- 后续投入转向检索与数据层（全量 48K POI、语义标注精细化、跨日换区规划——后者正是双峰中"远模式"的产品化）
- 若未来要兑现 LLM 的远桶优势：需要更强的模式判别（行程阶段/语义特征）或更大模型，当前 4B + 单特征门控不够
